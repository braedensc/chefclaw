...
| [M3: per-user budget/rate caps + paid Gemini tier](adr/2026-07-07-per-user-budget-caps.md) | 2026-07-07 | Wire the M2-readiness `users.monthly_budget_usd`/`max_attempts_per_day` columns into `check_budget` as per-user overrides of the global env cap (global parsed FIRST — fail-closed, a per-user cap can never re-enable disabled spend; the per-user read shares the gate's session so the concurrency-1 double-spend gate holds); `PATCH /api/admin/users/{id}/budget` behind `require_admin` (partial-update, `null` clears to global, positive-only, no migration — columns already exist); `/api/health` surfaces the EFFECTIVE cap + `budget_is_personal` + the caller's effective `model`/`paid_tier` so the Settings bar tracks the caller's real cap; paid Gemini tier is selectable GLOBALLY (`GEMINI_MODEL`) AND per-user (`users.paid_tier` → `GEMINI_PAID_MODEL`, one additive migration `f3a4b5c6d7e8`, set via the same admin endpoint; swap centralized in `extractor_settings_for_tier`, used by both worker + health) — already priced in `GEMINI_PRICING` so the budget gate bounds pro spend unchanged; plus per-user + daily-cap budget alerts and an admin cross-user spend view (`GET /api/admin/spend`) |
...
| [In-app admin config panel: runtime-policy overrides](adr/2026-07-07-admin-config-panel.md) | 2026-07-07 | A tiny `app_config` key/value table OVERRIDES env `Settings` at read time for a CLOSED allowlist of 8 runtime-policy flags (cover mode/real-covers, global model tier + media-resolution + escalation ceiling, global budget + daily-cap) — never secrets, never infra. Code-side registry is the allowlist + type/coercion + write-validation; `effective_settings(base, sm) -> base.model_copy(update=overrides)` reuses the `extractor_settings_for_tier` idiom so the ~10 read sites are untouched; injected at two edges — an async dep (`/api/health`, config API) and ONCE PER JOB in the serial worker, so changes land on the next job with NO restart (infra/secrets keep the restart boundary, shown read-only). Three-state rows (absent=inherit env / `""`=explicit disable-shadow / `"X"`=value); `PATCH {key:null}` deletes. Fail-closed preserved (cleared budget ⇒ `parse_budget` ConfigError ⇒ no paid calls; `media_resolution_max>base` validated on write AND re-checked per job); secrets never enter the table (422 on unknown key, status-only `configured` bool); append-only `config_audit` (old→new, `changed_by`) + log line. `GET`/`PATCH /api/admin/config` behind `require_admin`; one migration off `a1c2e3f4b5d6`; `/admin/config` panel section |
| [Push-based CD via GHCR](adr/2026-07-07-push-based-cd-ghcr.md) | 2026-07-07 | Merge to `main` → Actions builds the single image + pushes to GHCR → Actions SSHes the VPS (deploy key pinned to a `forced command` running `scripts/deploy.sh`) to redeploy. Build-off-box + immutable `@sha256` digest handoff (`compose.prod.yaml` pins migrate+api, pull-not-build); two-job trust split (build = `packages: write` + no SSH key; deploy = SSH key + `contents: read` only) with deny-all top-level `permissions` + SHA-pinned actions + never `pull_request`; on-box `deploy.sh` rails: capture-pullable-rollback-ref → backup → `ff-only` sync → pull → migrate-gated-BEFORE-swap → `up --no-build --no-deps` → unauth SPA-index health-gate → auto-rollback (`--pull never`) → exit-nonzero-reds-the-job; `CD_ENABLED` repo-variable master switch; rollback reverts CODE not SCHEMA (keep migrations expand-then-contract); health gate is liveness-only (future `/api/livez`); `.github/dependabot.yml` maintains the action pins |

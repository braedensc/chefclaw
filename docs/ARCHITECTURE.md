...
| [Design system & brand](adr/2026-07-06-design-system-and-brand.md) | 2026-07-06 | Neon night-market direction (bake-off-picked) as Tailwind v4 `@theme` tokens; claw-family puppy-chef mascot; real per-dish video poster frames via a best-effort ffmpeg worker stage, stored atomically and served owner-scoped (`has_cover` + authed `/cover` route — never the path); card facts projected verbatim from the document; restyle strictly inside the golden-selector contract; reduced-motion-guarded animation |

...
## The cover fields — a card sprite + a beauty-shot timestamp

Two more fields, both about the card COVER (decorative — they are assessments,
not captured food data, so they follow the looser tag-style rules, never the
faithful-capture rule):

- `cover_sprite_id` — pick the ONE sprite id from the **cover sprite catalog**
  (listed at the very end of this prompt) whose dish best matches this dish:
  the same dish if it's there, else the closest cuisine + form (a braise, a
  noodle soup, a stir-fry, a pizza…). Copy the id string EXACTLY as written in
  the catalog. If truly nothing is a reasonable match, use `null` — never invent
  an id that isn't in the catalog, and never pick `unknown-dish` yourself.
- `beauty_shot_timestamp_seconds` — the timestamp (in seconds, a number) of the
  single best "finished dish" beauty shot in the video: the plated, completed
  dish shown at its most appetizing (usually near the end). Give your best
  estimate as a number; use `null` if the video never clearly shows the finished
  dish.
...
```json
{
  ...
  "tags": [],
  "cover_sprite_id": null,
  "beauty_shot_timestamp_seconds": null
}
```
...
egg-based), a `cover_sprite_id` picked from the catalog (`egg-curds`, the
tomato-and-egg sprite), and a `beauty_shot_timestamp_seconds` for the finished
plate. Follow it exactly.

*(Always copy a `cover_sprite_id` that actually appears in the catalog below.)*

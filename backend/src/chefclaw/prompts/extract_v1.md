# Cooking-video recipe extraction — prompt v1
...
- **The ONLY judgments you may make are `difficulty` and `cuisine_type`.** These
  two fields are assessments and are allowed. Everything else is transcription —
  mark nothing else as inferred, and infer nothing else.
...
does not add a `source` block. Follow it exactly.
